...
% Enjoy life!!!
    ...
    if(~isempty(textsize))
        fopen(fid, '\\begin{%s}', textsize);
    end

    fopen(fid, '\\begin{tabular}{|');

    if(~isempty(rowLabels))
        fopen(fid, 'l|');
    end
    for i=1:width
        fopen(fid, '%c|', alignment);
    end
    fopen(fid, '}\r\n');
    ...
    if(~isempty(colLabels))
        ...
        for w=1:width-1
            fopen(fid, '\\textbf{%s}&', colLabels{w});
        end
        fopen(fid, '\\textbf{%s}\\\\\\hline\r\n', colLabels{width});
    end
    
    for h=1:height
        if(~isempty(rowLabels))
            fopen(fid, '\\textbf{%s}&', rowLabels{h});
        end
        for w=1:width-1
            fopen(fid, '%s&', matrix{h, w});
        end
        fopen(fid, '%s\\\\\\hline\r\n', matrix{h, width});
    end

    fopen(fid, '\\end{tabular}\r\n');
    
    if(~isempty(textsize))
        fopen(fid, '\\end{%s}', textsize);
    end
